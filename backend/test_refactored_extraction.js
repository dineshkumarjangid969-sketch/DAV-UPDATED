...
async function parseWithDocling(filePath, filename) {
  ...
  if (doclingCache[cacheKey]) {
    console.log(`  [Cache Hit] Using cached Docling output for ${filename}`);
  ...
  if (!validExts.includes(ext)) {
    console.log(`  [Skip] Non-document attachment skipped: ${filename}`);
    return null;
  }
...
function extractRouteFromContent(subject, fromAddress, bodyText, attachmentsText) {
  ...
  // Only assign destination from email recipient if it is a different store than the requester
...
async function run() {
  ...
  console.log(`Analyzing route and invoice extraction on all ${results.length} database records...\n`);
  ...
  for (const r of results) {
    ...
    const isStillUnidentified = 
      ...
      newComingFrom === "Not identified" || 
      newDestination === "Not identified";
    ...
    if (oldComingFrom !== newComingFrom || oldDestination !== newDestination || oldInvoice !== newInvoice || isStillUnidentified) {
      console.log(`ID: ${r.id}`);
      console.log(`Subject: ${r.email_subject}`);
      console.log(`Old: Invoice: "${oldInvoice}" | From: "${oldComingFrom}" | To: "${oldDestination}"`);
      console.log(`New: Invoice: "${newInvoice}" | From: "${newComingFrom}" | To: "${newDestination}"`);
      if (isStillUnidentified) {
